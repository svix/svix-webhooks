...
sealed class IngestSourceInConfig {
    ...
    @VariantName("merge")
    data class Merge(val merge: MergeConfig) : IngestSourceInConfig() {
        override fun toJsonElement() = Json.encodeToJsonElement(MergeConfig.serializer(), merge)
    }
    ...
    @VariantName("vgs")
    data class Vgs(val vgs: VgsConfig) : IngestSourceInConfig() {
        override fun toJsonElement() = Json.encodeToJsonElement(VgsConfig.serializer(), vgs)
    }
    ...
    companion object {
        private val typeMap =
            mapOf<String, (JsonElement) -> IngestSourceInConfig>(
                ...
                "merge" to
                    { config ->
                        Merge(Json.decodeFromJsonElement(MergeConfig.serializer(), config))
                    },
                "meta" to
                    { config ->
                ...
                "vgs" to
                    { config ->
                        Vgs(Json.decodeFromJsonElement(VgsConfig.serializer(), config))
                    },
            )
