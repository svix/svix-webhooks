...
OPENAPI_CODEGEN_IMAGE = "ghcr.io/svix/openapi-codegen:20260722-388"
...
def get_docker_binary() -> str:
    ...
    if docker_binary is None:
        print("Please install docker or podman to run the codegen", file=sys.stderr)
        exit(1)
...
def log_generated_files(generated_paths: list[list[str]]):
    ...
    if len(files_not_part_of_codegen) != 0:
        print("\n".join(sorted(files_not_part_of_codegen)), file=sys.stderr)
        raise Exception(
