...
public class StreamingStream {
    private final SvixHttpClient client;

    public StreamingStream(SvixHttpClient client) {
        this.client = client;
    }
    ...
    public ListResponseStreamOut list() throws IOException, ApiException {
        ...
        return this.list(new StreamingStreamListOptions());
    }

    /** List of all the organization's streams. */
    public ListResponseStreamOut list(final StreamingStreamListOptions options)
    ...
    public StreamOut create(final StreamIn streamIn) throws IOException, ApiException {
        return this.create(streamIn, new StreamingStreamCreateOptions());
    }

    /** Creates a new stream. */
    public StreamOut create(final StreamIn streamIn, final StreamingStreamCreateOptions options)
            throws IOException, ApiException {
        HttpUrl.Builder url = this.client.newUrlBuilder().encodedPath("/api/v1/stream");
