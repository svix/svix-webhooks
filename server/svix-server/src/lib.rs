...
/// Whether events from this target are produced by the OTLP export path itself.
///
/// Exporting a log record emits log records of its own, which would then be exported, and so
/// on: the collector keeps receiving traffic even when the service is idle. The OTel SDK
/// suppresses this, but the suppression rides on the OpenTelemetry `Context`, which
/// `hyper`/`tonic` don't propagate across the transport's thread boundaries.
///
/// These events still go to stdout; they're only excluded from OTLP export.
///
/// TODO: remove this once <https://github.com/open-telemetry/opentelemetry-rust/issues/2877>
/// is fixed and we've upgraded to a release containing the fix.
fn is_otlp_export_target(target: &str) -> bool {
    let crate_name = target.split("::").next().unwrap_or(target);
    crate_name.starts_with("opentelemetry")
        // The transport stack used by the OTLP exporter. Note this deliberately does not
        // match `tower_http`, which is svix-server's own HTTP layer.
        || crate_name.starts_with("hyper")
        || matches!(crate_name, "h2" | "tonic" | "tower")
}
...
pub fn setup_tracing(
    ...
    let mapped_logs = cfg
        ...
        .map(|addr| {
            ...
            // Copy fields from the enclosing `tracing` spans onto each log record, so
            // context like `org_id`/`app_id`/`msg_id` is searchable in the collector the
            // same way it is in the stdout logs.
            let layer = OpenTelemetryTracingBridge::builder(&provider)
                .with_tracing_span_attributes(TracingSpanAttributes::all())
                .build()
                // Exclude events generated by the export path itself, to avoid a feedback loop.
                .with_filter(tracing_subscriber::filter::filter_fn(|meta| {
                    !is_otlp_export_target(meta.target())
                }));
