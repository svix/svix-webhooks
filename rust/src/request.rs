...
impl Request {
    ...
    pub fn with_body_param<T: serde::Serialize>(
        mut self,
        param: T,
    ) -> Self {
    ...
    async fn execute_with_backoff(
        ...
        let retry_schedule = match &conf.retry_schedule {
            ...
            None => &std::iter::successors(
                ...
                |last_backoff| Some(MAX_BACKOFF.min(*last_backoff * 2)),
            )
    ...
    fn build_request(
        self,
        conf: &Configuration,
    ) -> Result<http1::Request<Full<Bytes>>, Error> {
        ...
        for (k, v) in self.path_params {
            ...
            let percent_encoded_path_param_value = utf8_percent_encode(
                &v,
                PATH_SEGMENT,
            )
            .to_string();
            path = path.replace(
                &format!("{{{k}}}"),
                &percent_encoded_path_param_value,
            );
        }
