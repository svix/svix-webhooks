/*
 * Svix API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.1.1
 *
 * Generated by: https://openapi-generator.tech
 */

#[allow(unused_imports)]
use crate::models;
#[allow(unused_imports)]
use serde::{Deserialize, Serialize};

/// GoogleCloudStorageConfig : Configuration for a Google Cloud Storage sink.
/// Write stream events into the named bucket using the supplied Google Cloud
/// credentials.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct GoogleCloudStorageConfig {
    #[serde(rename = "bucket")]
    pub bucket: String,
    /// Google Cloud Credentials JSON Object as a string.
    #[serde(rename = "credentials")]
    pub credentials: String,
}

impl GoogleCloudStorageConfig {
    /// Configuration for a Google Cloud Storage sink.  Write stream events into
    /// the named bucket using the supplied Google Cloud credentials.
    pub fn new(bucket: String, credentials: String) -> GoogleCloudStorageConfig {
        GoogleCloudStorageConfig {
            bucket,
            credentials,
        }
    }
}
