...
@dataclass
class StreamSinkListOptions(BaseOptions):
...
@dataclass
class StreamSinkCreateOptions(BaseOptions):
...
@dataclass
class StreamSinkRotateSecretOptions(BaseOptions):
    idempotency_key: t.Optional[str] = None
...
class StreamSinkAsync(ApiBase):
    async def list(
        self, stream_id: str, options: StreamSinkListOptions = StreamSinkListOptions()
    ) -> ListResponseStreamSinkOut:
