...
from .api_internal.autoconfig import (
    Autoconfig,
    AutoconfigAsync,
)
...
class AutoConfig:
    ...
    def subscribe(self) -> EndpointOut:
        ...
        if self._autoconfig_id is not None:
            return Autoconfig(self._client, self._httpx_client).endpoint.subscribe(
                self._app_id,
                self._autoconfig_id,
                self._endpoint,
            )
...
class AutoConfigConsumer:
    ...
    async def subscribe_async(self) -> DestinationOut:
        ...
        if self._autoconfig_id is not None:
            destination = await AutoconfigAsync(
                self._client, self._httpx_async_client
            ).destination.subscribe(
                self._app_id,
                self._autoconfig_id,
    ...
    def _get_sink_id(self) -> str:
        ...
        dest_id = (
            Autoconfig(self._client, self._httpx_client)
            .get(
                self._app_id,
                self._autoconfig_id,
            )
    ...
    async def _get_sink_id_async(self) -> str:
        ...
        if self._autoconfig_id is None:
            raise AutoConfigError("v2 tokens set autoconfig_id")
        dest_id = (
            await AutoconfigAsync(self._client, self._httpx_async_client).get(
                self._app_id,
                self._autoconfig_id,
            )
        ).dest_id
